...
// 함수안에 함수가 있으면 안에 함수는 부모함수를 볼 수 있다.
function first() {
  let x = 10;
...
// 전역의 오염을 막기 위해
// const counter = number()로 counter가 number()에 참조돼 있으므로
// garbage collector가 접근하지 못한다.
// 누구도 접근하지 못하게 하는 것을 closuer이라고 한다.(새로운 환경이 구성됨)
function number() {
  let count = 0;
